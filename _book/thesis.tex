% Modified by Siobhon Egan (M2020) for usage as honours thesis, Murdoch University
%
...
\date{Novemeber}
\division{College of Science, Health and Engineering}
\advisor{}
\institution{Murdoch University}
\reporttype{thesis}
\degreeorunit{degree}
\degreename{Bachelor of Science Honours (Molecular Biology)} 
...
\department{Medical, Molecular and Forensic Sciences}
...
\toprule
\endhead
\(^\circ\)C & Degrees celsius\tabularnewline
\(\mu L\) & Microlitre\tabularnewline
DNA & Deoxyribonucleic acid\tabularnewline
mL & Millilitre\tabularnewline
ML & Maximum likelihood\tabularnewline
MLST & Multi-locu sequence typing\tabularnewline
mM & Millimolar\tabularnewline
PCR & Polymerase chain reaction\tabularnewline
UV & Ultraviolet\tabularnewline
WA & Western Australia\tabularnewline
\bottomrule
\end{longtable}
\hypertarget{preface}{%
...
You can use this space to include any details on animal ethics, permits, grants, project overview etc.

\hypertarget{welcome-to-r-markdown}{%
\section*{Welcome to R Markdown}\label{welcome-to-r-markdown}}
\addcontentsline{toc}{section}{Welcome to R Markdown}
...
\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{include\_graphics}\NormalTok{(}\DataTypeTok{path =} \StringTok{"figure/random{-}field.png"}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
\begin{figure}
...
If you wish to use a short figure caption for the table of figures use \texttt{fig.scap="short\ name"}, this is what will appear in the TOF, then use \texttt{fig.cap="long\ caption\ title\ and\ description\ of\ data"} to include the longer caption you wish to appear under the figure.
\begin{Shaded}
\begin{Highlighting}[]
...
\end{verbatim}
\begin{figure}
\includegraphics{thesis_files/figure-latex/delaysboxplot-1} \caption[Mean Delays by Airline]{Bar graph showing the mean flight departure delays by airline from Portland for 2014. Only one airline was successful in decreasing the delay time (HA).}\label{fig:delaysboxplot}
\end{figure}
...
\NormalTok{  devtools}\OperatorTok{::}\KeywordTok{install\_github}\NormalTok{(}\StringTok{"thomas{-}fung/thesisdownmq"}\NormalTok{)}
\KeywordTok{library}\NormalTok{(thesisdownmq)}
\end{Highlighting}
\end{Shaded}
